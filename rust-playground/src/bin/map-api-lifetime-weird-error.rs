...
pub trait MapApiExt<'me, 'd, K>
where
    K: MapKey,
{
    async fn upsert_value(&mut self, key: K, value: K::V) -> (K::V, K::V);
}

// #[async_trait::async_trait]
// impl<'me, 'd, K, T> MapApiExt<'me, 'd, K> for T
// where
//     T: MapApi<'me, 'd, K>,
//     K: MapKey,
// {
...
where
    ...
    // &'d Level: MapApiRO<'d, K>,
    &'d mut Level: MapApi<'d, 'd, K>,
    for<'e> &'e Level: MapApiRO<'e, K>,
    // for<'him> &'him mut Level: MapApi<'him, 'him, K>,
    'me: 'd,
{
...
async fn main() {
...

    {
        ...
        {
            let r = lm.to_ref();
            let got = MapApiRO::get(r, &k()).await;
            println!("LevelMap::get() got: {:?}", got);
        }
        ...
        {
            let r = lm.to_ref();
            let got = MapApiRO::range(r, k()..).await;
            let x = got.collect::<Vec<_>>().await;
            println!("LevelMap::get() got: {:?}", x);
        }
    }
}
